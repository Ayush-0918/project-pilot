  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] min-h-[500px] border rounded-2xl sm:rounded-3xl overflow-x-hidden glass-panel relative" style={{ borderColor: 'var(--border-subtle)' }}>
      ...
      <div className="flex-1 flex flex-col justify-between h-full bg-gradient-to-b from-transparent to-[#0a0728]/10 relative min-w-0">
        ...
        <div className="p-3 sm:p-4 border-t space-y-2 bg-[#050214]/60 backdrop-blur-md shrink-0" style={{ borderColor: 'var(--border-subtle)' }}>
          {/* Quick Prompts */}
          <div className="flex items-center space-x-2 overflow-x-auto pb-1 no-scrollbar">
            {suggestedPrompts.map((prompt, idx) => (
              <button
                ...
                className="text-[10px] sm:text-xs whitespace-nowrap px-3 py-1.5 rounded-full border border-white/10 bg-white/5 hover:bg-white/10 text-slate-300 transition-colors shrink-0 cursor-pointer min-h-[32px]"
              >
                {prompt.label}
              </button>
            ))}
          ...
          {/* Input Area */}
          <div className="flex items-center space-x-2 bg-slate-900/80 border border-white/10 rounded-xl sm:rounded-2xl p-1.5 sm:p-2">
            <button
              onClick={() => fileInputRef.current?.click()}
            ...
            <textarea
              ...
              rows={1}
              className="flex-1 bg-transparent border-none text-xs sm:text-sm text-slate-100 placeholder-slate-500 focus:outline-none resize-none max-h-24 py-1.5"
            />
            ...
            <Button
              ...
              className="h-9 sm:h-10 px-3 sm:px-4 text-xs font-semibold rounded-lg cursor-pointer min-h-[36px]"
            >
